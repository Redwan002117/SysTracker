...
/**
 * Parse GitHub release markdown body into structured sections.
 * Handles common heading patterns: ## Features, ### Bug Fixes, etc.
 */
function parseChangelog(body: string): ParsedChangelog {
    ...
    let currentSection: keyof ParsedChangelog = 'other';

    const matchSection = (line: string): keyof ParsedChangelog | null => {
        const l = line.toLowerCase();
        if (/new features?|what.?s new|added|feature/i.test(l)) return 'features';
        if (/bug fix|bugfix|fixed|fixes|patch/i.test(l)) return 'bugfixes';
        if (/improve|enhance|update|upgrade|performance|refactor|polish/i.test(l)) return 'improvements';
        if (/breaking change|migration|incompatible/i.test(l)) return 'breaking';
        return null;
    };
    ...
    for (const raw of lines) {
        ...
        if (line.startsWith('-') || line.startsWith('*') || line.startsWith('+')) {
            ...
            // Try to infer section from inline emoji/keywords if we're in 'other'
...
const SECTIONS: ChangeSection[] = [
    { key: 'features', label: 'New Features', icon: <Sparkles size={15} />, color: 'text-blue-700', bg: 'bg-blue-50', border: 'border-blue-200' },
    { key: 'improvements', label: 'Improvements', icon: <ZapIcon size={15} />, color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
    { key: 'bugfixes', label: 'Bug Fixes', icon: <Bug size={15} />, color: 'text-rose-700', bg: 'bg-rose-50', border: 'border-rose-200' },
    { key: 'breaking', label: 'Breaking Changes', icon: <AlertTriangle size={15} />, color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
    { key: 'other', label: 'Other Changes', icon: <CheckCircle size={15} />, color: 'text-slate-700', bg: 'bg-slate-50', border: 'border-slate-200' },
];
                    ...
                    >
                        {items.map((item, i) => (
                            <li key={i} className="flex items-start gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                                <span className={`mt-0.5 flex-shrink-0 w-1.5 h-1.5 rounded-full mt-1.5 ${section.bg.replace('50', '400').replace('bg-', 'bg-')}`} />
                                <span>{item}</span>
                            </li>
                        ))}
                    </motion.ul>
...
export default function DownloadPage() {
    ...
    useEffect(() => {
        Promise.all([
            fetch(`https://api.github.com/repos/${REPO}/releases?per_page=20`).then(r => r.json()),
    ...
    const changelog = useMemo(() =>
        selectedRelease ? parseChangelog(selectedRelease.body) : null,
        [selectedRelease]
    );

    const hasStructuredChangelog = changelog &&
        (changelog.features.length + changelog.bugfixes.length + changelog.improvements.length + changelog.breaking.length) > 0;
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                        <div>
                            ...
                            >
                                <Tag size={12} />
                                {loading ? 'Fetching releases...' : `${releases.length} releases on GitHub`}
                            </motion.div>
                            <motion.h1
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
                        ...
                        >
                            <Github size={16} />
                            All releases on GitHub <ExternalLink size={12} />
                        </motion.a>
                    </div>
                ...
                {!loading && !error && (
                    <div className="grid lg:grid-cols-5 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-2">
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider px-1 mb-3">All Versions</p>
                                ...
                                >
                                    <div className="flex items-center justify-between mb-1.5">
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            {release.prerelease && (
                                                <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-semibold">Pre</span>
                                            )}
                                            {!release.prerelease && idx === 0 && (
                                            ...
                                            <div className="flex items-start justify-between gap-4">
                                                <div>
                                                    <div className="flex items-center gap-2 flex-wrap mb-1">
                                                        ...
                                                        {selectedRelease.prerelease && (
                                                            <span className="text-xs bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full font-bold">Pre-release</span>
                                                        )}
                                                    </div>
                                            ...
                                            {selectedRelease.body ? (
                                                <div>
                                                    ...
                                                    {hasStructuredChangelog ? (
                                                        ...
                                                        /* Fallback: display raw body nicely */
                                                ...
                                                <div className="text-center py-8 text-slate-400 text-sm">
                                                    ...
                                                    <br />
                                                    <a href={selectedRelease.html_url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline mt-1 inline-block">View on GitHub</a>
                                                </div>
                                            )}
                                                ...
                                                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                                    <div>
                                                        ...
                                                        <p className="text-slate-400 text-sm">
                                                            Open an issue on GitHub — we review every report.
                                                        </p>
                                                    </div>
                                                    <div className="flex gap-2 flex-shrink-0">
                                                        <a
                                                            href={`https://github.com/${REPO}/issues/new?labels=bug&template=bug_report.md`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                        ...
                                                        <a
                                                            href={`https://github.com/${REPO}/issues/new?labels=enhancement`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                ...
                <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pb-24">
                    <div className="grid sm:grid-cols-3 gap-5">
                        {[
                            ...
                            {
                                icon: <Users2 size={22} className="text-purple-500" />,
                            ...
                            {
                                icon: <ZapIcon size={22} className="text-emerald-500" />,
